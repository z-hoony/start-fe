...
function setPhotos(index) {
  ...
  let str = '';
  for (
    let i = index * numberOfColumn;
    i < todayPhoto.length && i < (index + 1) * numberOfColumn;
    i += 1
  ) {
    str +=
      '<div class="outer"><img class="image" src="' +
      todayPhoto[i].img +
      '"><div class="desc">' +
      todayPhoto[i].title +
      '</div></div>';
...
function setTotalPageCount(count) {
  const totalCount = document.getElementById('total_count');
  totalCount.innerHTML = ` / ${count}`;
}

const totalCount = Math.ceil(todayPhoto.length / numberOfColumn);
